...
# fields_final = ['S.no.', 'concentration']
# filename_final = "final_data.csv"
# with open(filename_final, 'w') as csvfile:
#     csvwriter = csv.writer(csvfile)
#     csvwriter.writerow(fields_final)
...
while True:
        ...
        if (diff)>=60:
            ...
            read_eye=pd.read_csv(r"C:\Users\hp\Desktop\capstone\eye_blink.csv")
            read_eye['concentration']= label_encoder.fit_transform(read_eye['concentration'])
            temp=list(read_eye.iloc[-1:,0])
            eye.extend(temp)
            # here ATTENTIVE=0
            eye_att_min_vise=[]
            for i in range(0,6):
                if(len(eye)<=i):
                    ...
                    att_eye=[100 if eye[i]==0 else 0]
                    att_eye=att_eye[0]
                eye_att_min_vise.append(att_eye)
            print(eye_att_min_vise)
            read_head=pd.read_csv(r"C:\Users\hp\Desktop\capstone\head_movement.csv")
            read_head['concentration']= label_encoder.fit_transform(read_head['concentration'])
            temp1=list(read_head.iloc[-1000:,0])
            head.extend(temp1)
            head_att_min_vise=[]
            for i in range(1,7):
                att_head=(1000-sum(head[(i-1)*1000:i*1000+1]))/1000
                if(len(head)<i*1000):
                    att_head=0
                head_att_min_vise.append(att_head*100)
            
            read_exp=pd.read_csv(r"C:\Users\hp\Desktop\capstone\example.txt")
            temp2=list(read_exp.iloc[-200:,1])
            expression.extend(temp2)
            exp_att_min_vise=[]
            for i in range(1,7):
                att_exp=sum(expression[(i-1)*200:i*200+1])/200
                if(len(expression)<i*200):
                    att_exp=0
                exp_att_min_vise.append(att_exp)
                
            
            total_att=[]
            for i in range(len(eye_att_min_vise)):
                he=(eye_att_min_vise[i]+head_att_min_vise[i])/2
                total_att.append((exp_att_min_vise[i]+he)/2)
            final_df=pd.DataFrame(total_att)
            final_df.to_csv("final_data.txt",header=False)
